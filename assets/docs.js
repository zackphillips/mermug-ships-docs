...
// scripts/build-index.mjs in the Pages workflow on every push to main), and
// each .md file is fetched verbatim and rendered with marked. Adding a
// document is therefore just "commit a .md file" — including from the GitHub
// web UI.
//
...
// scripts/build-index.mjs) so the file stays predictable to diff.
...
// Relative links and images in a document resolve against the document's own
// directory, exactly as GitHub renders them, so a file reads the same in the
// repository and here. Returns the repo-relative path, or null for anything
// absolute (a scheme, a leading slash, a protocol-relative URL).
const DUMMY_ORIGIN = 'https://docs.invalid/';
function resolveFromDoc(docPath, href) {
  if (/^([a-z][a-z0-9+.-]*:|\/)/i.test(href)) return null;
  const url = new URL(href, DUMMY_ORIGIN + docPath);
  return { path: decodeURIComponent(url.pathname.slice(1)), hash: url.hash };
}

function decorateImages(entry) {
  for (const img of el.article.querySelectorAll('img[src]')) {
    const resolved = resolveFromDoc(entry.path, img.getAttribute('src'));
    if (resolved) img.setAttribute('src', resolved.path);
...
function decorateLinks(entry) {
  for (const link of el.article.querySelectorAll('a[href]')) {
    ...
    const resolved = resolveFromDoc(entry.path, href);
    if (resolved && /^docs\/.+\.md$/i.test(resolved.path)) {
      const slug = resolved.path.replace(/^docs\//, '').replace(/\.md$/i, '');
      link.setAttribute('href', `?doc=${encodeURIComponent(slug)}${resolved.hash}`);
...
// a single .md file lands under docs/.
function showEmptyState() {
  ...
  el.article.innerHTML = `
    <div class="docs-empty">
      ...
      <p>
        Commit any <code>.md</code> file under <code>docs/</code> in
        <a href="https://github.com/${C.GITHUB_REPO}" target="_blank" rel="noopener noreferrer">${C.GITHUB_REPO}</a>
        and it appears here after the next deploy, usually within a minute.
      </p>
    </div>`;
  renderNav();
}
...
async function init() {
  ...
  try {
    ...
    el.article.innerHTML =
      ...
      `<br /><span class="docs-error__hint">The Pages workflow builds ` +
      `<code>docs/index.json</code> on every deploy. Locally, run ` +
      `<code>node scripts/build-index.mjs</code> first.</span></p>`;
    return;
  }
